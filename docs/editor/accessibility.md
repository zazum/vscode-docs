...
## Custom color accessibility

You can search for extensions in Visual Studio Marketplace that are compatible for color vision deficiency. Select the  Extensions icon `kbstyle(Ctrl+Shift+X)` to access the Marketplace extension search bar and enter "colorblind" to populate relevant options.

![Extension icon is highlighted in UI](images/accessibility/accessibility-extension-marketplace.png)

To change the [color theme](/docs/getstarted/themes.md), go to **File** > **Preferences** > **Color Theme** `kbstyle(Ctrl+K Ctrl+T)`.

![The term "colorblind" in the Extension Marketplace search bar](images/accessibility/accessibility-extension-marketplace-search.png)
